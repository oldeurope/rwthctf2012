class StringCode
  def code(asm)
    asm.asm do
      ...
      function :memset do #t1 => ptr, t2 => length, t3 => value
        push t1,t2,t3
          add t2,t1 #t2 is now ptr to end of buffer
          label :loop
            mov [t1],t3
            inc t1
          if_then(t1,:<,t2) do
      ...
      function :strcopy do #t1 src,t2 dst, t3 len
        push t1,t2,t3
        add t3,t1 # t3 marks end
        label :loop
        if_then([t1],:!=,0) do
          if_then(t1,:<,t3) do #end of buffer
            mov [t2],[t1]
            inc t1
            inc t2
            jmp_to :loop
